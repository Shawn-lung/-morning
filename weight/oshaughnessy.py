...
def evaluate(individual):
    criteria = {
        ('Normalized EPS', 'Normalized PE Ratio'): individual[0],
        ('Normalized EPS', 'Normalized ROE'): individual[1],
        ('Normalized PE Ratio', 'Normalized ROE'): individual[2]
    }
...
optimal_criteria = {
    ('Normalized EPS', 'Normalized PE Ratio'): best_individual[0],
    ('Normalized EPS', 'Normalized ROE'): best_individual[1],
    ('Normalized PE Ratio', 'Normalized ROE'): best_individual[2]
}

ahp_model = ahpy.Compare('Criteria', optimal_criteria)
print("Optimal Weights (O'Shaughnessy):")
print(ahp_model.report())

# 保存最佳權重到CSV文件
weights_df = pd.DataFrame.from_dict(optimal_criteria, orient='index', columns=['Weight'])
weights_df.to_csv('oshaughnessy_optimal_weights.csv')
